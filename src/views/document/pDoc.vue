<template>
  <div class="app-container">
    ...
    <el-table :data="list" style="width: 100%" @selection-change="handleSelectionChange" ref="multipleTable">
      ...
      <el-table-column prop="id" align="center" label="操作" width="300" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button size="small" type="success" @click="shareFolder(scope.row)">分享</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div align="center">
    ...
    <el-dialog title="新建文件夹" :visible.sync="createFolderDialog" :before-close="createFolderDialogClose">
      <!--新建文件夹表单-->
      <el-form :model="createFolderForm" ref="createFolderForm" label-width="15%" size="30%">
    ...
    <el-dialog title="分享" :visible.sync="shareFolderDialog" :before-close="shareFolderDialogClose">
      <!--<el-input v-model="this.multipleSelection" placeholder="分享人员id动态数组" style='width: 50%;'></el-input>-->
      <!--分享表单-->
      <!--<el-button class="filter-item" type="danger" icon="delete" v-show="true" @click="selectBatchSharePerson">批量选择分享人员</el-button>-->
      <el-form :model="shareFolderForm" :rules="shareRules" ref="shareFolderForm" label-width="15%" size="30%">
        <el-form-item label="分享类型" prop="shareType">
          <!--<el-select filterable clearable style="width: 200px" class="filter-item" v-model="shareFolderForm.shareType" placeholder="分享类型">-->
            <el-select filterable clearable style="width: 200px" class="filter-item" v-model="shareFolderForm.shareType" >
            <el-option v-for="item in listShareType" :key="item.id" :label="item.value" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="清空人员" >
           <el-button size="small" type="success" @click="clearSharePerson">清空人员</el-button>
        </el-form-item>
        <el-form-item label="分享人员" prop="sharePersonNames">
        </el-form-item>
         <el-form-item label="所选择的分享文件或文件夹id" prop="shareRefId">
        </el-form-item>
        <!--<el-table :data="list" style="width: 100%" @selection-change="handleSelectionChange" ref="multipleTable">
          <el-table-column type="selection" width="55">
          </el-table-column>
          ...
          <el-table-column prop="id" align="center" label="操作" width="100" fixed="right">
            <template slot-scope="scope">
              <el-button size="small" type="danger" v-show="false" @click="deleteGoods(scope.row.id)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>-->
        <!-- 部门列表 -->
        <el-table :data="selectShareList" style="width: 100%">
          <el-table-column type="expand">
            <template slot-scope="props">
              <el-form label-position="left" inline class="demo-table-expand" @selection-change="handleSelectionChange" >
                <span v-for="user in props.row.users">
                <el-form-item type="selection" width="55" v-show = "true">
                </el-form-item>
                ...
                <el-form-item width="50" fixed="right">
                  <el-button type="danger" size="mini" @click="addSharePerson(user.userName)">添加分享人员</el-button>
                </el-form-item><br/>
                </span>
              </el-form>
            </template>
          ...
          <!--<el-table-column prop="deptCode" align="center" label="操作" width="0" fixed="right">
            <template slot-scope="scope">
              <el-button size="small" type="danger" v-show="false" @click="deleteGoods(scope.row.deptCode)">删除</el-button>
            </template>
          </el-table-column>-->
        </el-table>
        <el-button style='margin-bottom:20px;margin-left:35%;' type="primary" icon="document" @click="submitshareFolderForm('shareFolderForm')">提交</el-button>
      </el-form>
    </el-dialog>
  </div>
</template>
<script>
  ...
  import { shareFolder } from '@/api/document/eDoc'
  //员工表
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        shareFolderDialog: false, //修改文件夹dialog
        shareFolderForm: Object.assign({}, shareTemp), //需要修改的文件夹表单属性
        //添加修改文件夹表单验证
        folderRules: {
          name: [
        ...
        shareRules: {
          personName: [
            { required: false, message: "请输入授权人名称", trigger: "blur" }
          ], //新建文件夹名称
          shareType: [
            { required: false, message: "请填入父文件夹id", trigger: "change" }
          ]
    ...
    methods: {
      ...
      fetchData() {
        ...
        // console.log(this.listQuery.remark)
      ...
      getPerson() {
        ...
        paginationBranch(this.listQuery).then(response => {
          const data = response.data.responseData;
          this.selectShareList = data.list;
          this.totalCount = data.total;
          this.listLoading = false;
        });
      },
    ...
    */
      ...
      */
      ...
      addSharePerson(data) {
        console.log(this.multipleSelection,"111111111111111--")
        for(let i=0;i<this.multipleSelection.length;i++){
          if(this.multipleSelection[i] == data){
            alert("此人员的添加已存在")
            return
          }
        }
        //Vue.set(this.multipleSelection, this.multipleSelection.length, text);
        ...
        console.log(this.multipleSelection,"22222222222----")
      },
      // selectBatchSharePerson() {
      //   const ids = [];
      //   this.multipleSelection.forEach(function (item) {
      //      ids.push(item.id);
      //   });
      //   this.deleteGoods(ids);
      //   //console.log(ids, "1112222222222221111")
      //   return ids;
      // },
      // submitshareFolderForm(formName) {
      //   this.$refs[formName].validate(valid => {
      //     this.selectBatchSharePerson()
      //     if (valid) {
      //       this.$confirm("确认添加", "提示", {
      //         confirmButtonText: "确定",
      //         cancelButtonText: "取消",
      ...
      //       })
      //         .then(() => {
      //           shareFolder(this.shareFolderForm, ids).then(response => {
      //             this.listLoading = false;
      //             if (response.data.statusCode == 200) {
      //               this.$notify({
      //                 title: "成功",
      ...
      submitshareFolderForm(formName) {
        this.$refs[formName].validate(valid => {
          //let ids = this.selectBatchSharePerson();
          //console.log(ids, "--------------------------")
          //console.log(this.shareFolderForm.sharePersonIds, "--------------------------")
          //this.shareFolderForm.sharePersonIds = ids
          this.shareFolderForm.sharePersonNames = this.multipleSelection
          shareFolder(this.shareFolderForm).then(response => {
            this.listLoading = false;
            if (response.data.statusCode == 200) {
              this.$notify({
                title: "成功",
    ...
    * 更新dialog关闭事件
    */
      shareFolderDialogClose(done) {
        this.shareFolderDialog = false;
        this.$refs["shareFolderForm"].resetFields();
      },
      /**
      * 点击更新按钮事件
      */
      shareFolder(row) {
        //this.shareFolderForm = row;
        this.shareFolderDialog = true;
        console.log(this.shareFolderForm)
        this.shareFolderForm.shareRefId = row.id;
      },
      //删除文件夹
